...
async function fetchYamlFile(path: string) {
  const asObject: z.infer<typeof YamlTrickTableSchemaZod> = YamlTrickTableSchemaZod.parse(
  ...
  if (asObject.recommendedPrerequisites) {
    // @ts-expect-error Expected to fail. Gets validated before Return thanks to Zod
    asObject.recommendedPrerequisites = asObject.recommendedPrerequisites.map((prerequisiteId) => [
      prerequisiteId,
      'official',
    ]);
  }
  ...
  if (asObject.variationOf) {
    // @ts-expect-error Expected to fail. Gets validated before Return thanks to Zod
    asObject.variationOf = asObject.variationOf.map((variationId) => [variationId, 'official']);
  }
...
function findDuplicateKeys(allTricks: z.infer<typeof DbTricksTableZod>[]) {
  ...
  return Object.entries(idToTrickLookup)
    .filter(([_, trickList]) => trickList.length > 1)
    .map(([id, trickList]) => ({
      id,
    ...
    if (trick.recommendedPrerequisites) {
      trick.recommendedPrerequisites.forEach((prerequisitePrimaryKey, i) => {
        if (prerequisitePrimaryKey[1] !== 'official') {
          issues.push([
            trick.id,
            `All recommendedPrerequisites must limit themselves to official tricks (not the case for ${
              i + 1
            }. Entry)`,
          ]);
          ...
          issues.push([
            ...
            `${
              i + 1
            }. Entry in recommendedPrerequisites references an ID that does not exist (id: ${
              prerequisitePrimaryKey[0]
            }).`,
          ]);
        } else if (prerequisitePrimaryKey[0] == trick.id) {
          issues.push([
            trick.id,
            `${i + 1}. Entry in recommendedPrerequisites references itself (id: ${
              prerequisitePrimaryKey[0]
            }).`,
          ]);
        }
    ...
    if (trick.variationOf) {
      trick.variationOf.forEach((variationPrimaryKey, i) => {
        if (variationPrimaryKey[1] !== 'official') {
          issues.push([
            trick.id,
            `All variationOf-Entries must limit themselves to official tricks (not the case for ${
              i + 1
            }. Entry)`,
          ]);
          ...
          issues.push([
            trick.id,
            `${i + 1}. Entry in variationOf references an ID that does not exist (id: ${
              variationPrimaryKey[0]
            }).`,
          ]);
        } else if (variationPrimaryKey[0] == trick.id) {
          issues.push([
            trick.id,
            `${i + 1}. Entry in variationOf references itself (id: ${variationPrimaryKey[0]}).`,
          ]);
        }
      });
    }
...
export default async function viteGetAllTricks() {
  ...
  const parsedYamlFiles = await Promise.allSettled(
    allFilePaths.map((trickFilePath) => fetchYamlFile(trickFilePath))
  );
  const erroredFiles = parsedYamlFiles
    .map((trickYamlFile, i) =>
      trickYamlFile.status === 'rejected'
        ? { error: trickYamlFile.reason, path: allFilePaths[i] }
        : (undefined as never)
    )
    .filter(Boolean);
  const goodFiles = parsedYamlFiles
    .map((trickYamlFile) =>
      trickYamlFile.status === 'fulfilled' ? trickYamlFile.value : (undefined as never)
    )
    .filter(Boolean);
