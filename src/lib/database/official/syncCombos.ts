...
async function moveCombo(comboId: number, from: DbCombo["comboStatus"], to: DbCombo["comboStatus"]) {
...
async function handleUnarchivingCombos(combos: DbCombo[]) {
  return await Promise.allSettled( combos.map( async e => {
    await moveCombo(e.id, "official", "archived")
...
export default async function syncCombos() {
  ...
  if(combosThatHaveBeenRemoved.length > 0) {
    ...
    const errors = result.map( e => e.status === "rejected" ? e.reason : undefined as never).filter( e => e)
    if(errors.length > 0) {
