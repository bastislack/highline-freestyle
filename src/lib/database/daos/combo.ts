...
export class Combo implements DbObject {
  ...
  public set yearEstablished(val) {
    ...
    this.dataFromDatabase.yearEstablished = DbCombosTableZod._def
      .shape()
   ...
   */
  constructor(
    private dataFromDatabase: z.infer<typeof DbCombosTableZod>,
  ...
  public async persist(refetch = true): Promise<string | true> {
    ...
    try {
      ...
      Object.keys(this.#modified.dataFromDatabase).forEach(
        (dbCombo) => (this.#modified.dataFromDatabase[dbCombo as 'name'] = true)
      );
      Object.keys(this.#modified.metadata).forEach(
        (dbMeta) => (this.#modified.metadata[dbMeta as 'notes'] = true)
      );
