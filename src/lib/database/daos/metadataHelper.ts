...
type PrimaryKey = [MetadataTable["id"], MetadataTable["entityStatus"], MetadataTable["entityCategory"]]
...
export async function putDefault(db: MainDatabase, primaryKey: PrimaryKey) {
  ...
  const newObject: MetadataTable = {
    id, entityStatus, entityCategory,
    isFavourite: false
  }
...
export async function getMetadata(db: MainDatabase, primaryKey: PrimaryKey, createDefaultIfNotExist = true) {
