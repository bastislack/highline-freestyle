...
import { primaryKeysMatch, StickableStatus, PrimaryKey } from '@/lib/utils';
...
export class Trick implements DbObject {
  ...
  public get changed() {
    return [
      ...Object.values(this.#modified.dataFromDatabase),
      ...Object.values(this.#modified.metadata),
    ].some((e) => e);
  }
  ...
  public set difficultyLevel(val) {
    ...
    this.dataFromDatabase.difficultyLevel = DbTricksTableZod._def
      .shape()
      .difficultyLevel.parse(val);
  }
  ...
  public set yearEstablished(val) {
    ...
    this.dataFromDatabase.yearEstablished = DbTricksTableZod._def
      .shape()
      .yearEstablished.parse(val);
  }
  ...
  public set recommendedPrerequisites(val) {
    ...
    this.dataFromDatabase.recommendedPrerequisites = DbTricksTableZod._def
      .shape()
      .recommendedPrerequisites.parse(val);
  }
  ...
  public set showInSearchQueries(val) {
    ...
    this.dataFromDatabase.showInSearchQueries = DbTricksTableZod._def
      .shape()
      .showInSearchQueries.parse(val);
  }
   ...
   */
  constructor(
    private dataFromDatabase: z.infer<typeof DbTricksTableZod>,
    private metadataFromDatabase: z.infer<typeof DbMetadataZod>,
  ...
  public async persist(refetch = true): Promise<string | true> {
    ...
    if (this.#modified.persisting) {
      console.warn(
        'Did you call .persist() twice? Did you await a response from the persist call?'
      );
    ...
    try {
      ...
      //                                                                                              lmao.
      Object.keys(this.#modified.dataFromDatabase).forEach(
        (e) => (this.#modified.dataFromDatabase[e as 'alias'] = true)
      );
      Object.keys(this.#modified.metadata).forEach(
        (e) => (this.#modified.metadata[e as 'notes'] = true)
      );
  ...
  public async updateStatusPersistent(status: StickableStatus): Promise<PrimaryKey> {
    if (this.#modified.deleted) {
      throw new Error('Cannot update status! Trick has been deleted.');
    }

    if (this.#modified.persisting) {
      throw new Error('Cannot persist because this object is currently getting persisted!');
    }
    this.#modified.persisting = true;
    ...
    let newPrimaryKey: PrimaryKey = [this.primaryKey[0], this.primaryKey[1]];
      ...
      await this.db.transaction(
        ...
        async () => {
          ...
          allTricks.forEach((trick) => {
            ...
            const newPrerequisites = trick.recommendedPrerequisites.map(
