...
export default class Database {
  ...
  populateTricks = async () => {
    ...
    await this.db.versions.put({
      key: "predefinedTricksVersion",
      version: predefinedTricksVersion,
    });
  };
  ...
  getTrick = (id) =>
    this.db.userTricks.get(Number(id)).then((userTrick) => {
  ...
  saveTrick = (trick) => {
    ...
    if (trick.id) return this.db.userTricks.put(trick);
      ...
      return this.db.predefinedTricks
        .toCollection()
        .primaryKeys()
        .then((trickKeys) => {
          this.db.userTricks
  ...
  populateCombos = () => {
    return this.db.predefinedCombos.clear().then(() => {
      return this.getAllTricks().then((allTricks) => {
        ...
        const comboList = Papa.parse(predefinedCombos, {
          dynamicTyping: true,
        }).data;
        ...
        return this.db.predefinedCombos.bulkPut(combos).then(() => {
          ...
          return this.db.versions.put({
            key: "predefinedCombosVersion",
            version: predefinedCombosVersion,
          });
        });
      });
    });
  ...
  getAllCombos = () => {
    return this.db.userCombos.toArray().then((userCombos) => {
      ...
      return this.db.predefinedCombos
        .where("id")
        .noneOf(userKeys)
        .toArray()
        .then((preCombos) => preCombos.concat(userCombos.filter((combo) => !combo.deleted)));
    });
  };
      ...
      .then(() => {
        ...
        IDBExportImport.exportToJsonString(idbDatabase, function (err, jsonString) {
          if (err) {
            ...
            fileDownload(
              jsonString,
              "highline-freestyle.com".concat(date.toLocaleDateString().replaceAll("/", "-"), ".json")
            );
          }
        });
      })
