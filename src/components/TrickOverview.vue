...
const filteredTricks = ref([] as Trick[])
...
function compareTrickNames(a: Trick, b: Trick): number {
  if (a.alias && b.alias) {
...
function filterTricks(searchText: string): void {
  filteredTricks.value = allTricks.filter(trick => {
    return trick.alias?.toLowerCase().includes(searchText.toLowerCase()) ||
      trick.technicalName.toLowerCase().includes(searchText.toLowerCase())
  })
}
    ...
    <div v-for="level in Object.keys(tricksByDifficulty).length - 1" :key="level">
      ...
      <div class="p-4 grid grid-cols-3 xs:grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-3">
        <div v-for="trick in tricksByDifficulty[level as any]!" :key="trick.id"
          ...
          <p class="text-dark-gray text-center line-clamp-3 text-base/5 break-words">
