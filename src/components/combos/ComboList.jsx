...
const ComboList = ({sortOpt, scrollPosition, setScrollPosition}) => {
  ...
  const combos = useLiveQuery(
    () =>
      db.getAllCombos().then((c) => c.sort(sortingSchemes[sortOpt].sortFunc)),
    [sortOpt],
  );
  if (!combos || combos.length == 0) {
    return (
      <p>
        You have no saved combos. For now it is only possible to create a combo
        using the combo generator, we are working on supporting custom combos.
      </p>
    );
  }
  console.log(combos);
  ...
  return (
    <div className="justify-content-evenly">
      <div className="row">
        {combos.map((combo) => {
          let isFirst =
            sortingSchemes[sortOpt].attributeFunc(combo) !== current;
          ...
          if (isFirst && sortingSchemes[sortOpt].showCategory) {
            return [
              <div
                ...
                key={"header" + combo.id.toString()}
              >
                <h4>
                  {!sortingSchemes[sortOpt].attributeLast && current}{" "}
                  {sortingSchemes[sortOpt].catName}{" "}
                  {sortingSchemes[sortOpt].attributeLast && current}
                </h4>
              </div>,
              getComboDiv(combo),
