...
<template>
  <nav :class="navClassList">
    ...
    >
      ...
      <RouterLink to="/" class="md:text-2xl hidden md:block text-white font-heading hover:underline"
        >Highline Freestyle</RouterLink
      >
      <div class="flex-1" />
      <ul class="inline-flex flex-row gap-1 text-white font-prose">
        <li v-for="entry in links" :key="entry.to">
          <RouterLink :to="entry.to" class="rounded-lg md:text-xl hover:bg-black/20 p-3">{{
            entry.label
          }}</RouterLink>
        </li>
      </ul>
          ...
          >
            <div class="inline-flex flex-col divide-y dark:divide-slate-800">
              <button
                ...
                :key="locale.locale"
                class="p-4 hover:bg-black/10 flex flex-row items-center gap-3"
              >
    ...
    <div class="sm:hidden">
      <nav class="flex flex-row bg-primary/90 dark:bg-slate-800 backdrop-blur-sm items-center">
        <div class="flex-1">
          <button
            ...
            class="m-1 p-2 border border-white hover:bg-white/30 rounded-lg pointer-events-auto"
          >
      ...
      <div
        ...
        :class="`hidden fixed top-0 overflow-hidden left-0 w-full h-full group/wrapper pointer-events-auto ${
          isMobileNavExpanded ? 'active' : ''
        }`"
      >
        <div
          id="mobile-nav-background"
          @click="isMobileNavExpanded = false"
          class="h-full w-full bg-black/40 cursor-pointer group-[.active]/wrapper:block absolute hidden"
        />
        <div
